...
/// </summary>
    class AccDocumentItemListViewModel : BindableBase
    {
        #region Constructors
        ...
        public List<EditableAccDocumentItem> ItemsToBeDeleted { get; set; }
        ...
        {
            ItemsToBeDeleted = new List<EditableAccDocumentItem>();
            ...
            if (AccDocumentHeader != null)
            {
                ...
                AccDocumentHeader.Difference = AccDocumentHeader.SumDebit - AccDocumentHeader.SumCredit;
            }
            ...
            AccDocumentItems.CollectionChanged += (s, ea) =>
            {
                ...
                if (ea.Action == System.Collections.Specialized.NotifyCollectionChangedAction.Remove)
                {
                    ...
                    ItemsToBeDeleted.AddRange(ea.OldItems.Cast<EditableAccDocumentItem>());
                    return;
                }
        ...
        private async void OnSave()
        {
            ...
            IEnumerable<AccDocumentItem> accDocumentItems = new List<AccDocumentItem>();
            Mapper.Map(AccDocumentItems, accDocumentItems);
             _accDocumentItemsService.AddAccDocumentItemsAsync(accDocumentItems);
            if (ItemsToBeDeleted?.Any()==true)
            {
                var delete = _accDocumentItemsService.DeleteItemsAccDocumentItemAsync(ItemsToBeDeleted.Select(x => x.AccDocumentItemId));
                AccDocumentHeader.SumDebit -= ItemsToBeDeleted.Sum(x => x.Debit);
                AccDocumentHeader.SumCredit -= ItemsToBeDeleted.Sum(x => x.Credit);
            }
        }
